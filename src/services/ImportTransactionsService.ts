...
class ImportTransactionsService {
  public async execute(filePath: string): Promise<Transaction[]> {
    ...
    const addCategoryTitles = categories
      .filter(category => !existsCategoriesTitles.includes(category)) // procura nos titles se não tiver adiciona
      .filter((value, index, self) => self.indexOf(value) === index); // se houver repetido só adiciona um

    const newCategories = categoriesRepository.create(
      addCategoryTitles.map(title => ({
        // um map no add category
        title,
      })),
    );
